...
def load_app_config():
    ...
    try:
        with open('app_config_visage.json', 'r') as f:
            return json.load(f)
    except FileNotFoundError:
        st.error("App configuration file not found. Please ensure app_config_visage.json exists.")
        st.stop()
    except json.JSONDecodeError:
        st.error("Invalid app configuration file. Please check app_config_visage.json format.")
        st.stop()
...
def load_processed_knowledge_base():
    ...
    processed_dir = "processed_knowledge_base_visage"
    ...
    if not os.path.exists(processed_dir):
        ...
        ```bash
        python preprocess_knowledge_base_visage.py
        ```
        """)
        st.stop()
...
def search_live_pdfs(query: str, knowledge_base_dir: str = "knowledge_base_visage", max_results: int = 3) -> List[Dict]:
...
def display_knowledge_base_info(kb_data: dict):
    ...
    with st.expander("📚 Hybrid Knowledge Base Status", expanded=False):
        ...
        with col2:
            ...
            live_pdfs = glob.glob(os.path.join(live_pdf_dir, "*.pdf")) if os.path.exists(live_pdf_dir) else []
...
def main():
    ...
    with st.expander("🎯 Quick Start by Role", expanded=False):
        ...
        with col3:
            st.markdown("**👩‍⚕️ Healthcare Professionals**")
    ...
    # Chat input
    if prompt := st.chat_input("Ask about PACS administration, troubleshooting, configuration, or user management..."):
        # Add user message
        st.session_state.messages.append({"role": "user", "content": prompt})
    ...
    with col1:
        ...
        st.markdown("• **DICOM integration issues** → Consult your system administrator")
        st.markdown("• **Critical system problems** → Follow your organization's emergency IT procedures")
